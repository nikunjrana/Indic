...
import { mergeGeolocIplocFunc } from '../Globals/GlobalFunc';
import { getLocationPageName } from './locationHelper';
...
geolocation_newservice(service=1){
     ...
     if((!cookiedata || !cookieDataState) && !bot_request && service==1){
     ...
     return makeRequestLocation('POST','https://geoip.imimg.com/api/location',data,service)
    }
    else
    {
        if(service==2){
    ...
    xhr.onload = function () {
        if (this.status == 200 && this.readyState == 4) {
            ...
            if (p.Response.Code == 200) {
                ...
                else { cookieString = "gcniso=" + p.Response.Data.geoip_countryiso + "|gcnnm=" + p.Response.Data.geoip_countryname + "|gacrcy=" + p.Response.Data.geoip_accuracy + "|gip=" + p.Response.Data.geoip_ipaddress ; 
                cookieString1="state="+"";
            }
