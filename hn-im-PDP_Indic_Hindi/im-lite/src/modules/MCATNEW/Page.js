...
import mcatHelper from "../MCATNEW/utility/helper";
...
class Page extends Component { 
  constructor(props) {
    ...
    !window.location.href.includes('waId') ? '' : '';
    this.state = {
    ...
    loadPopUp() {
        // if(!this.state.PopUp) {import(/* webpackChunkName:"PopUp" */ '../CentralizePopUp/container/PopUpContainer').then((module) => {this.setState({ PopUp: module.default });})}
    }
    componentWillMount(){
    ...
    loadComponent(){
    // if(!this.state.footer){import(/* webpackChunkName:"Footer" */'../App/container/FooterContainer').then((module) => {this.setState({footer: module.default });});}
  ...
  processMcatData(data,rehit=false) {
    ...
    let cityName = this.props.params.cityORalias
      ...
      : this.props.location.pathname && this.props.location.pathname.split("/")&& this.props.location.pathname.split("/")[1] == "city" &&  this.props.location.pathname.split("/")[2] ? this.props.location.pathname.split("/")[2] : "";
  ...
  loadErrorView(code, response) {
    if (code == this.code5xx) {
      import(
        /* webpackChunkName:"Error5xx" */ "../MCATNEW/utility/Error5xx/Error5XX"
      ).then((module) => {
        this.hideLoader();
        this.setState({
    ...
    } else if(code == this.code404) {
      import(
        /* webpackChunkName:"Error404" */ "./../MCATNEW/utility/Error404/Error404"
      ).then((module) => {
        this.hideLoader();
        this.setState({
  ...
  // loadHeader(){
  //  if(!this.state.header) {import(/* webpackChunkName:"Header" */ '../Header/HeaderContainer').then((module) => {this.setState({header: module.default }); })}
  // }
  ...
  componentDidMount() {
    ...
    //         // this.loadHeader();
    //     } else {
    //         window.addEventListener('touchstart', this.loadHeader, { passive: true });
...
onPopHistory() {
  ...
  render() {
   ...
   return (
      <>
        {this.state.PopUp ?<this.state.PopUp pageUrl={window.location.href} pageName={"impcat"}/> : ""}
        {pageView}
      </>
   );
  }
