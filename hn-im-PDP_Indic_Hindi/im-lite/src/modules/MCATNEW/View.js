...
import "../MCATNEW/CSS/FirstFold.css";
import Listing from "../MCATNEW/utility/Listing";
...
loadEnqCallContainer() {
  // if (!this.state.EnqBlView) {import(/* webpackChunkName:"EnqBlPWA" */'../EnquiryBlForms/components/EnqBlMain').then((module) => {this.setState({ EnqBlView: module.default });})}
  // if (!this.state.CallView) {import(/* webpackChunkName:"CallPWA" */'../CallNow/container/CallNowContainer').then((module) => {this.setState({ CallView: module.default  });})}
  window.removeEventListener('touchstart', this.loadEnqCallContainer, { passive: true });
...
callNowClick(callObj) {
  ...
  connectionChecking() {
    if (navigator && navigator.connection && navigator.connection.effectiveType) {
      ...
      if (/\slow-2g|2g|slow-3g|3g/.test(connectionType)) {return true; }
      return false;}
    return false;
  }
  loadAdsJs() {
    // if (!this.state.AdsComponent) {import(/* webpackChunkName:"GoogleDFP" */ '../App/components/GoogleDFP').then((module) => {this.setState({ AdsComponent: module.default })});}
  ...
  componentDidMount() {
    ...
    if (!this.connectionChecking() && (this.viewCount <= 1) && !window.location.href.includes('waId')) {
      window.addEventListener('touchstart', this.loadEnqCallContainer, { passive: true });
    ...
    if (!this.state.BrandBillboard && this.props.mcatData.brandBillboardAdShow) {import(/* webpackChunkName:"BrandBillboard" */'../App/components/BrandBillboard').then((module) => {this.setState({ BrandBillboard: module.default });});}
    window.addEventListener('touchstart', this.loadComponent, { passive: true });
    window.addEventListener('scroll', this.loadComponent, { passive: true });
   window.addEventListener("scroll", this.loadAdsJs, { passive: true }); 

  }
  loadComponent(){
    // if (!this.state.AutofetchCont) {import(/* webpackChunkName:"Autofetch" */'../Mcat/components/Autofetch').then(module => { this.setState({ AutofetchCont: module.default })});}
    // if(!this.state.belowListing){import(/* webpackChunkName:"McatBelowlisting" */'../Mcat/components/BelowListing').then((module)=> {this.setState({belowListing: module.default });});}
    // if (!this.state.FeaturedCategoriesContainer) {import(/* webpackChunkName:"Autofetch" */ "../Widgets/Personalisation/FeaturedCategories/FeaturedCategoriesContainer").then((module) => {this.setState({ FeaturedCategoriesContainer: module.default }); });}
    window.removeEventListener("touchstart", this.loadComponent, { passive: true });
 ...
 componentWillUnmount(){
  ...
  componentDidUpdate(prevProps) {
    ...
    import(/* webpackChunkName:"BrandBillboard" */'../App/components/BrandBillboard').then((module) => {this.setState({ BrandBillboard: module.default });});
    }
  }
  ...
  render() {
    ...
    let adsResolutions = [[336, 280], [320, 150], [320, 200], [300, 150], [300, 100], [320, 250], [300, 200], [300, 250], [320, 100]]
    ...
    return (
      ...
      {this.state.AutofetchCont ? <this.state.AutofetchCont
        ...
        {/* {this.state.FeaturedCategoriesContainer ? (<this.state.FeaturedCategoriesContainer translatedText={this.props.translatedTxt} page="Impcat" count="10" queryText={"MCAT-Featured-Category-Product-Click|" + this.props.mcatData.tracking.lang.split("Lang")[1]}/>) :''} */}
        {this.state.EnqBlView && !this.state.showMiniPdp && (!this.props.mcatData.foreignCampaignMiniBl || (this.props.mcatData.foreignCampaignMiniBl && (this.props.country_value || this.props.country_value == '' || this.props.country_value == "IN")))  ? <this.state.EnqBlView prop={this.mblProp} showEnqPopup={this.state.showEnqPopup} out_tot_unq_count={this.props.mcatData.totalProductCount}/> :''}
