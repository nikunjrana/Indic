...
export default class PinchZoomPan extends React.Component {
    ...
    handleTouchStart = event => {
        this.props.toggleSwiper(true);  //hsingh
    ...
    handleTouchEnd = event => {
        this.props.toggleSwiper(false);  //hsingh
        this.cancelAnimation();
    ...
    pinchChange(touches) {
        ...
        const scale = this.lastPinchLength
            ? this.state.scale * length / this.lastPinchLength //sometimes we get a touchchange before a pointerdown when pinching
            : this.state.scale;
    ...
    zoomIn(midpoint, speed = 0, factor = 0.1) {
        ...
        this.props.toggleSwiper(true);  //hsingh applied to stop the swiper while doing pinch zoom
        midpoint = midpoint || {
    ...
    zoomOut(midpoint) {
        ...
        this.props.toggleSwiper(false); //hsingh applied to start the swiper while ending pinch zoom
    ...
    constrainAndApplyTransform(requestedTop, requestedLeft, requestedScale, tolerance, speed = 0) {
        ...
        if (isEqualTransform(transform, this.state)) {
            return (this.props.toggleSwiper(false),  false); //hsingh
        }

        this.applyTransform(transform, speed);
        return (this.props.toggleSwiper(true), true);  //hsingh
    }
